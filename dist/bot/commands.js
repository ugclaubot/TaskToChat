...
function isManager(ctx) {
    ...
    return userId === config_1.config.manager.telegramId || config_1.config.manager.telegramId === '';
}
...
function formatTaskLine(task, index) {
    ...
    const due = task.due_date ? ` (Due: ${(0, taskParser_1.formatDueDate)(new Date(task.due_date))})` : '';
...
function formatTaskBlock(task) {
    ...
    if (task.employee_name)
        lines.push(`   👤 ${task.employee_name}`);
    if (task.due_date)
        lines.push(`   📅 Due: ${(0, taskParser_1.formatDueDate)(new Date(task.due_date))}`);
...
function registerCommands(bot) {
    ...
    bot.start((ctx) => {
        ctx.reply('👋 Welcome to *ChatFlow*!\n\n' +
            ...
            '`task: @Person do something by Friday`', { parse_mode: 'Markdown' }).catch(() => {
            ctx.reply('Welcome to ChatFlow! Type /help for commands.');
        });
    });
    // /help
    bot.help((ctx) => {
        ctx.reply('📋 *ChatFlow Commands*\n\n' +
            '*Task Creation:*\n' +
            ...
            '/employees — List all employees', { parse_mode: 'Markdown' });
    });
    ...
    bot.command('addemployee', (ctx) => {
        ...
        if (!args) {
            return ctx.reply('❌ Usage: `/addemployee Name @username 919810000000`\n\n' +
                'Example: `/addemployee John Doe @johndoe 919810123456`', { parse_mode: 'Markdown' });
        }
        ...
        try {
            const employee = (0, employee_1.createEmployee)(name, username, whatsapp);
            ctx.reply(`✅ Employee added!\n\n` +
                `👤 *${employee.name}*\n` +
                (employee.telegram_username ? `📱 @${employee.telegram_username}\n` : '') +
                (employee.whatsapp_number ? `💬 WhatsApp: ${employee.whatsapp_number}\n` : '') +
                `🆔 ID: ${employee.id}`, { parse_mode: 'Markdown' });
        }
        catch (err) {
    ...
    bot.command('employees', (ctx) => {
        ...
        const lines = employees.map((e, i) => `${i + 1}. *${e.name}*` +
    ...
    bot.command('tasks', (ctx) => {
        ...
        let tasks;
        if (arg) {
            const employee = (0, employee_1.findEmployee)(arg);
            if (!employee) {
                return ctx.reply(`❌ Employee "${arg}" not found. Check /employees for the list.`);
            }
            tasks = (0, task_1.getAllTasksWithEmployees)({ employeeId: employee.id }).filter((t) => t.status !== 'completed');
            if (tasks.length === 0) {
                return ctx.reply(`✅ No pending tasks for *${employee.name}*!`, { parse_mode: 'Markdown' });
            }
            const lines = tasks.map((t, i) => formatTaskLine(t, i));
            ctx.reply(`📋 *Tasks for ${employee.name}* (${tasks.length})\n\n${lines.join('\n')}`, { parse_mode: 'Markdown' });
        }
        else {
            tasks = (0, task_1.getAllTasksWithEmployees)().filter((t) => t.status !== 'completed');
            if (tasks.length === 0) {
    ...
    bot.command('mytasks', (ctx) => {
        ...
        const employee = (0, employee_1.getEmployeeByUsername)(username);
        if (!employee) {
            return ctx.reply(`❌ Your username @${username} isn't in the employee directory. Ask the manager to add you with /addemployee.`);
        }
        const tasks = (0, task_1.getAllTasksWithEmployees)({ employeeId: employee.id }).filter((t) => t.status !== 'completed');
        if (tasks.length === 0) {
            return ctx.reply(`✅ You have no pending tasks, ${employee.name}! Great work!`);
        }
        const lines = tasks.map((t, i) => formatTaskLine(t, i));
        ctx.reply(`📋 *Your Tasks, ${employee.name}* (${tasks.length})\n\n${lines.join('\n')}\n\nUse /done <task number or keywords> to mark complete.`, { parse_mode: 'Markdown' });
    });
    // /done <ID or keywords>
    bot.command('done', (ctx) => {
        ...
        if (/^\d+$/.test(arg)) {
            ...
            (0, task_1.completeTask)(taskId, `Marked done by @${ctx.from?.username ?? 'unknown'}`);
        ...
        const username = ctx.from?.username;
        const employee = username ? (0, employee_1.getEmployeeByUsername)(username) : null;
        const matches = (0, task_1.findTasksByKeywords)(arg, employee?.id);
        if (matches.length === 0) {
        ...
        if (matches.length > 1) {
            const list = matches
                ...
                .map((t) => `• #${t.id}: ${t.title}`)
                .join('\n');
            return ctx.reply(`⚠️ Multiple tasks match "${arg}". Please use a task ID:\n\n${list}\n\nThen: /done <ID>`, { parse_mode: 'Markdown' });
        }
        const task = matches[0];
        (0, task_1.completeTask)(task.id, `Marked done by @${ctx.from?.username ?? 'unknown'}`);
    ...
    bot.command('overdue', (ctx) => {
        (0, task_1.updateOverdueTasks)();
        const tasks = (0, task_1.getAllTasksWithEmployees)({ status: 'overdue' });
        if (tasks.length === 0) {
            return ctx.reply('✅ No overdue tasks! Everything is on track.');
        }
        const lines = tasks.map((t) => `⚠️ *#${t.id}* ${t.title}\n` +
            `   👤 ${t.employee_name ?? 'Unassigned'} | Due: ${t.due_date ? (0, taskParser_1.formatDueDate)(new Date(t.due_date)) : 'Unknown'}`);
        ctx.reply(`🚨 *Overdue Tasks* (${tasks.length})\n\n${lines.join('\n\n')}`, { parse_mode: 'Markdown' });
    });
}
